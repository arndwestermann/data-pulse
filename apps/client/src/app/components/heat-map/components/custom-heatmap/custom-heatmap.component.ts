...
export class CustomHeatmapComponent extends BaseChartComponent {
	...
	getDimension(value: string | number | Array<string | number>, index = 0, N: number, L: number): number {
		...
		if (Array.isArray(value) && typeof index === 'number') {
			return this.getDimension(value[index], undefined, N, L);
		}
	...
	setColors(): void {
		this.colors = new ColorHelper(this.scheme, ScaleType.Ordinal, this.valueDomain, (value: string | number): string => {
			if (typeof value === 'string') return '#fff';
	...
	getLegendOptions(): LegendOptions {
		return {
			...
			title: this.scaleType === ScaleType.Ordinal ? this.legendTitle : '',
